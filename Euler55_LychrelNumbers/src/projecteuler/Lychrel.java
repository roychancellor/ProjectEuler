...
public class Lychrel {
	...
	public static void main(String[] args) {
		...
		System.out.println("The number of Lychrel numbers below 10,000 is " + numLychrel);
	}
	
	public static boolean isLychrelNumber(int n) {
		BigDecimal sum = new BigDecimal(n).add(new BigDecimal(RoyUtils.reverseDigitsOfANumber(n)));
		StringBuilder sumStr = new StringBuilder(sum.toPlainString());
		int count = 0;
		while(count < 50) {
			if(RoyUtils.isPalindrome(sumStr.toString())) {
				return false;
			}
			count++;
			sum = sum.add(new BigDecimal(sumStr.reverse().toString()));
			sumStr.replace(0, sumStr.length(), sum.toPlainString());
		}
		System.out.println(n + " is Lychrel");
		return true;
	}
